...
@Component({
  selector: 'app-interview-schedule-details',
  templateUrl: './interview-schedule-details.component.html',
  styleUrls: ['./interview-schedule-details.component.css']
})
export class InterviewScheduleDetailsComponent implements OnInit {
  ...
  getInterviewDetails(): any {
    ...
    this.candidateService.getInterviewDetails(input).subscribe(response =>{
      this.successInterviewDetails(response.output)
    },
    error => { this.errorHandler(error) }
    )
  }
  ...
  successInterviewDetails(response:any){
    this.loading = false;
    this.interviewDetails = response.list1[0];
    // if (_.intersection(this.disabilities, this.interviewDetails.specialAssistDeclaration.split(',')).length > 0)
    //     this.disabilitiesArray =  this.interviewDetails.specialAssistDeclaration.split(',');
        ...
        "inputs": [
          {
            ...
            "specialAssistDeclaration" : this.disabilityList,
