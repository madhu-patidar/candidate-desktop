...
export class AddressCheckComponent implements OnInit {
  ...
  NavigateToNext(){
    this.router.navigate(['/candidates/my-profile/criminal-check'])
  }
  ...
  initForm(){
    this.addressDocDetailsForm = new FormGroup({
      aadhaarCard: new FormControl( {value : this.aadhaarDetails ? this.aadhaarDetails.documentNumber : '', disabled: this.isEntityDisabled && this.aadhaarDetails}, [Validators.required, Validators.minLength(12), Validators.maxLength(12)] ),
      passport: new FormControl({value : this.passportDetails ? this.passportDetails.documentNumber : '',  disabled: this.isEntityDisabled && this.passportDetails}),
      validTill: new FormControl({value:  this.passportDetails &&   this.passportDetails.validTill && JSON.stringify(this.passportDetails.validTill) !== '{}' ? this.datePipe.transform(this.passportDetails.validTill, 'yyyy-MM-dd') : '',  disabled: this.isEntityDisabled && this.passportDetails}),
      pan: new FormControl({value: this.panDetails ? this.panDetails.documentNumber : '',  disabled: this.isEntityDisabled && this.panDetails}, [Validators.required]),
      addressTypeCurrent: new FormControl({value: this.aadhaarDetails && (this.aadhaarDetails.addressType !== 'P')? true : false,  disabled: this.isEntityDisabled && this.aadhaarDetails}),
      addressTypeParmanet: new FormControl({value:  this.aadhaarDetails && (this.aadhaarDetails.addressType !== 'C')? true : false,  disabled: this.isEntityDisabled && this.aadhaarDetails} ),
  ...
  setDuration(duration?){
    ...
    for (let i = 0; i < this.subEntityExistingDetails.length; i++) {
      ...
      console.log('durationDetailst', this.subEntityExistingDetails);
  }
  ...
  onSubmit(status?: any) {
    ...
    if(this.duration){
      ...
      if(this.isEntityDisabled){
        if(this.duration[this.duration.length-1] && (this.duration[this.duration.length-1]['toDate'] && this.duration[this.duration.length-1]['fromDate'] && (moment(this.duration[this.duration.length-1]['toDate']) < moment(this.duration[this.duration.length-1]['fromDate'])))){
        ...
        for (let index = 0; index < this.durationDetails.length; index++) {
          if(this.durationDetails[index] && (this.durationDetails[index]['toDate'] && this.durationDetails[index]['fromDate'] && (moment(this.durationDetails[index]['toDate']) < moment(this.durationDetails[index]['fromDate'])))){
            // this.toastr.error("To Date should be greater than From Date", '', {
            //   closeButton: true
            // });
            this.alertPopService.error("To Date should be greater than From Date");
    ...
    if(this.isInvalid()){
      ...
      if(this.personalDetails.jobCountryCode === 'IND' || this.personalDetails.jobCountry === 'India'){
        let errors = this.addressDocDetailsForm.controls.aadhaarCard.errors
        if(errors && (errors.maxlength || errors.minlength)){
          this.alertPopService.error("Invalid Aadhaar card number");
          return false
        }
      }
    ...
    // if((this.personalDetails.jobCountryCode === 'IND' || this.personalDetails.jobCountry === 'India') && (this.addressDocDetailsForm.value.passport && _.intersection( ['','NA', '-'], this.addressDocDetailsForm.value.passport.trim().split()).length < 1 ) && (!this.addressDocDetailsForm.value.validTill || (this.addressDocDetailsForm.value.validTill.trim() === '')) ){
    ...
    if(this.personalDetails.jobCountryCode === 'IND' ||         this.personalDetails.jobCountry === 'India' ){
      ...
      if(this.addressDocDetailsForm.value.passport && this.addressDocDetailsForm.value.passport !=='' ){
        this.input.inputs.subEntityDetails[0]['passport'] = this.addressDocDetailsForm.value.passport;
  ...
  */
  ...
  passportUpload(output: UploadOutput, fileDetails?): void {
    ...
    if (output.type === 'allAddedToQueue') {
      ...
      if(output.file && this.filesForPassport && this.filesForPassport.length>0)
      this.filesForPassport.forEach(file =>{
        if(file.name === output.file.name && file.size === output.file.size){
          flag = 1
        }
      })
  ...
  panUpload(output: UploadOutput, fileDetails?): void {
    ...
    if (output.type === 'allAddedToQueue') {
      ...
      if(output.file && this.filesForPan && this.filesForPan.length>0)
      this.filesForPan.forEach(file =>{
        if(file.name === output.file.name && file.size === output.file.size){
          flag = 1
        }
      })
  ...
  aadharUpload(output: UploadOutput, fileDetails?): void {
    ...
    if (output.type === 'allAddedToQueue') {
      ...
      if(output.file && this.filesForAadharcard && this.filesForAadharcard.length>0)
      this.filesForAadharcard.forEach(file =>{
        if(file.name === output.file.name && file.size === output.file.size){
          flag = 1
        }
      })
